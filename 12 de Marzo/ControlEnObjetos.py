...
#                botonCancelarMedicion.grid(column=1, row=0)                
        ...
        def MenuPrincipal():
            raiz1.destroy()
            self.Continuar()
        botonMenuPrincipal = tk.Button(raiz1, text = 'Menu', command = MenuPrincipal)
    ...
    def CalcularTiempoDeMedicionALambdaFija(self, numeroDeConstantesDeTiempo,
        ...
        TiempoMonocromador = abs(longitudDeOndaFija_nm-self.experimento.mono.posicion)/self.experimento.mono.velocidadNmPorSegundo
        for i in range(0, len(VectorPosicionInicialSMC_mm)):
            if i>0:
    ...
    def CalcularTiempoDeMedicionCompleta(self, numeroDeConstantesDeTiempo,
        ...
        TiempoSMCInicial = abs(VectorPosicionInicialSMC_mm[0]-self.experimento.smc.posicion)/self.experimento.smc.velocidadMmPorSegundo
        TiempoDeRetornoSMC = abs(VectorPosicionFinalSMC_mm[largoVectorSMC-1]-VectorPosicionInicialSMC_mm[0])/self.experimento.smc.velocidadMmPorSegundo
        TiempoMonocromadorInicial = abs(VectorLongitudDeOndaInicial_nm[0]-self.experimento.mono.posicion)/self.experimento.mono.velocidadNmPorSegundo
        for i in range(0, len(VectorPosicionInicialSMC_mm)):
            if i>0:
                TiempoMuertoSMC = TiempoMuertoSMC + (VectorPosicionInicialSMC_mm[i]-VectorPosicionFinalSMC_mm[i-1])/self.experimento.smc.velocidadMmPorSegundo
            CantidadDeMediciones = CantidadDeMediciones + abs(VectorPosicionFinalSMC_mm[i]-VectorPosicionInicialSMC_mm[i])/VectorPasoSMC_mm[i]
            TiempoDeDesplazamientoPorPaso = VectorPasoSMC_mm[i]/self.experimento.smc.velocidadMmPorSegundo
        ...
        for j in range(0, len(VectorLongitudDeOndaInicial_nm)):
            if j>0:
                TiempoMuertoMono = TiempoMuertoMono + abs(VectorLongitudDeOndaInicial_nm[j]-VectorLongitudDeOndaFinal_nm[j-1])/self.experimento.mono.velocidadNmPorSegundo
            CantidadDeMovimientos = CantidadDeMovimientos + abs(VectorLongitudDeOndaFinal_nm[j]-VectorLongitudDeOndaInicial_nm[j])/VectorPasoMono_nm[j]
            TiempoDeDesplazamientoPorPaso = VectorPasoMono_nm[j]/self.experimento.mono.velocidadNmPorSegundo
    ...
    def ProgramaMedicionAPosicionFija(self):
        ...
        def SiguienteDePosicionFijaSMC():
            ...
            Var1 = tk.IntVar()
            tk.Checkbutton(raiz2, text='X', variable=Var1).grid(column=0,row=18)
            Var2 = tk.IntVar()
            tk.Checkbutton(raiz2, text='Y', variable=Var2).grid(column=0,row=19)
            Var3 = tk.IntVar()
            tk.Checkbutton(raiz2, text='R', variable=Var3).grid(column=0,row=18, sticky=tk.E)
            Var4 = tk.IntVar()
            tk.Checkbutton(raiz2, text='\u03B8', variable=Var4).grid(column=0,row=19, sticky=tk.E)
            ...
            def IniciarMedicion():
                ValoresAGraficar = (Var1.get(),Var2.get(),Var3.get(),Var4.get())
                self.grafico = Grafico(ValoresAGraficar,1,0)
                VectorLongitudDeOndaInicial_nm = np.zeros(numeroDeSubintervalos)
                VectorLongitudDeOndaFinal_nm = np.zeros(numeroDeSubintervalos)
                VectorPasoMono_nm = np.zeros(numeroDeSubintervalos)
                if numeroDeSubintervalos>=1:
                    VectorLongitudDeOndaInicial_nm[0] = float(textoPosicionInicial1.get())
                    VectorLongitudDeOndaFinal_nm[0] = float(textoPosicionFinal1.get())
                    VectorPasoMono_nm[0] = float(textoPaso1.get())
                if numeroDeSubintervalos>=2:
                    VectorLongitudDeOndaInicial_nm[1] = float(textoPosicionInicial2.get())
                    VectorLongitudDeOndaFinal_nm[1] = float(textoPosicionFinal2.get())
                    VectorPasoMono_nm[1] = float(textoPaso2.get())
                if numeroDeSubintervalos>=3:
                    VectorLongitudDeOndaInicial_nm[2] = float(textoPosicionInicial3.get())
                    VectorLongitudDeOndaFinal_nm[2] = float(textoPosicionFinal3.get())
                    VectorPasoMono_nm[2] = float(textoPaso3.get())
                if numeroDeSubintervalos>=4:
                    VectorLongitudDeOndaInicial_nm[3] = float(textoPosicionInicial4.get())
                    VectorLongitudDeOndaFinal_nm[3] = float(textoPosicionFinal4.get())
                    VectorPasoMono_nm[3] = float(textoPaso4.get())
                if numeroDeSubintervalos>=5:
                    VectorLongitudDeOndaInicial_nm[4] = float(textoPosicionInicial5.get())
                    VectorLongitudDeOndaFinal_nm[4] = float(textoPosicionFinal5.get())
                    VectorPasoMono_nm[4] = float(textoPaso5.get())
                if numeroDeSubintervalos>=6:
                    VectorLongitudDeOndaInicial_nm[5] = float(textoPosicionInicial6.get())
                    VectorLongitudDeOndaFinal_nm[5] = float(textoPosicionFinal6.get())
                    VectorPasoMono_nm[5] = float(textoPaso6.get())
                if numeroDeSubintervalos>=7:
                    VectorLongitudDeOndaInicial_nm[6] = float(textoPosicionInicial7.get())
                    VectorLongitudDeOndaFinal_nm[6] = float(textoPosicionFinal7.get())
                    VectorPasoMono_nm[6] = float(textoPaso7.get())
                if numeroDeSubintervalos>=8:
                    VectorLongitudDeOndaInicial_nm[7] = float(textoPosicionInicial8.get())
                    VectorLongitudDeOndaFinal_nm[7] = float(textoPosicionFinal8.get())
                    VectorPasoMono_nm[7] = float(textoPaso8.get())
                if numeroDeSubintervalos>=9:
                    VectorLongitudDeOndaInicial_nm[8] = float(textoPosicionInicial9.get())
                    VectorLongitudDeOndaFinal_nm[8] = float(textoPosicionFinal9.get())
                    VectorPasoMono_nm[8] = float(textoPaso9.get())
                if numeroDeSubintervalos>=10:
                    VectorLongitudDeOndaInicial_nm[9] = float(textoPosicionInicial10.get())
                    VectorLongitudDeOndaFinal_nm[9] = float(textoPosicionFinal10.get())
                    VectorPasoMono_nm[9] = float(textoPaso10.get())
                    

                tiempoDeMedicion = str(self.CalcularTiempoDeMedicionAPosicionFijaSMC(numeroDeConstantesDeTiempo,posicionFijaSMC_mm,VectorLongitudDeOndaInicial_nm,VectorLongitudDeOndaFinal_nm,VectorPasoMono_nm))
                raizMedicion = tk.Tk()
                raizMedicion.title('Pump and Probe Software - Midiendo a Posicion de la plataforma de retardo fija')
                ...
                self.experimento.grafico = self.grafico
                self.experimento.MedicionAPosicionFijaSMC(nombreArchivo,numeroDeConstantesDeTiempo,posicionFijaSMC_mm,VectorLongitudDeOndaInicial_nm,VectorLongitudDeOndaFinal_nm,VectorPasoMono_nm)
                nombreGrafico = nombreArchivo.replace('.csv','')
                self.grafico.GuardarGrafico(nombreGrafico)
                labelEstado = tk.Label(raizMedicion, text="Medicion Finalizada. El archivo ha sido guardado con el nombre: " + nombreArchivo)
                labelEstado.grid(column=0, row=2)
        ...
        def MenuPrincipal():
            raiz2.destroy()
            self.Continuar()
        botonMenuPrincipal = tk.Button(raiz2, text = 'Menu', command = MenuPrincipal)
        botonMenuPrincipal.grid(column=0, row=4)
    ...
    def ProgramaMedicionCompleta(self):
        ...
        def Siguiente():
            ...
            Var1 = tk.IntVar()
            tk.Checkbutton(raiz3, text='X', variable=Var1).grid(column=0,row=18)
            Var2 = tk.IntVar()
            tk.Checkbutton(raiz3, text='Y', variable=Var2).grid(column=0,row=19)
            Var3 = tk.IntVar()
            tk.Checkbutton(raiz3, text='R', variable=Var3).grid(column=0,row=18, sticky=tk.E)
            Var4 = tk.IntVar()
            tk.Checkbutton(raiz3, text='\u03B8', variable=Var4).grid(column=0,row=19, sticky=tk.E)
            ...
            def IniciarMedicion():
                ...
                if numeroDeSubintervalosMono>=1:
                    VectorLongitudDeOndaInicial_nm[0] = float(textoPosicionInicial1Mono.get())
                    VectorLongitudDeOndaFinal_nm[0] = float(textoPosicionFinal1Mono.get())
                    VectorPasoMono_nm[0] = float(textoPaso1Mono.get())
                if numeroDeSubintervalosMono>=2:
                    VectorLongitudDeOndaInicial_nm[1] = float(textoPosicionInicial2Mono.get())
                    VectorLongitudDeOndaFinal_nm[1] = float(textoPosicionFinal2Mono.get())
                    VectorPasoMono_nm[1] = float(textoPaso2Mono.get())
                if numeroDeSubintervalosMono>=3:
                    VectorLongitudDeOndaInicial_nm[2] = float(textoPosicionInicial3Mono.get())
                    VectorLongitudDeOndaFinal_nm[2] = float(textoPosicionFinal3Mono.get())
                    VectorPasoMono_nm[2] = float(textoPaso3Mono.get())
                if numeroDeSubintervalosMono>=4:
                    VectorLongitudDeOndaInicial_nm[3] = float(textoPosicionInicial4Mono.get())
                    VectorLongitudDeOndaFinal_nm[3] = float(textoPosicionFinal4Mono.get())
                    VectorPasoMono_nm[3] = float(textoPaso4Mono.get())
                if numeroDeSubintervalosMono>=5:
                    VectorLongitudDeOndaInicial_nm[4] = float(textoPosicionInicial5Mono.get())
                    VectorLongitudDeOndaFinal_nm[4] = float(textoPosicionFinal5Mono.get())
                    VectorPasoMono_nm[4] = float(textoPaso5Mono.get())
                if numeroDeSubintervalosMono>=6:
                    VectorLongitudDeOndaInicial_nm[5] = float(textoPosicionInicial6Mono.get())
                    VectorLongitudDeOndaFinal_nm[5] = float(textoPosicionFinal6Mono.get())
                    VectorPasoMono_nm[5] = float(textoPaso6Mono.get())
                if numeroDeSubintervalosMono>=7:
                    VectorLongitudDeOndaInicial_nm[6] = float(textoPosicionInicial7Mono.get())
                    VectorLongitudDeOndaFinal_nm[6] = float(textoPosicionFinal7Mono.get())
                    VectorPasoMono_nm[6] = float(textoPaso7Mono.get())
                if numeroDeSubintervalosMono>=8:
                    VectorLongitudDeOndaInicial_nm[7] = float(textoPosicionInicial8Mono.get())
                    VectorLongitudDeOndaFinal_nm[7] = float(textoPosicionFinal8Mono.get())
                    VectorPasoMono_nm[7] = float(textoPaso8Mono.get())
                if numeroDeSubintervalosMono>=9:
                    VectorLongitudDeOndaInicial_nm[8] = float(textoPosicionInicial9Mono.get())
                    VectorLongitudDeOndaFinal_nm[8] = float(textoPosicionFinal9Mono.get())
                    VectorPasoMono_nm[8] = float(textoPaso9Mono.get())
                if numeroDeSubintervalosMono>=10:
                ...
                VectorPasoSMC_mm = np.zeros(numeroDeSubintervalosSMC)
                if numeroDeSubintervalosSMC>=1:
                    VectorPosicionInicialSMC_mm[0] = float(textoPosicionInicial1SMC.get())
                    VectorPosicionFinalSMC_mm[0] = float(textoPosicionFinal1SMC.get())
                    VectorPasoSMC_mm[0] = float(textoPaso1SMC.get())
                if numeroDeSubintervalosSMC>=2:
                    VectorPosicionInicialSMC_mm[1] = float(textoPosicionInicial2SMC.get())
                    VectorPosicionFinalSMC_mm[1] = float(textoPosicionFinal2SMC.get())
                    VectorPasoSMC_mm[1] = float(textoPaso2SMC.get())
                if numeroDeSubintervalosSMC>=3:
                    VectorPosicionInicialSMC_mm[2] = float(textoPosicionInicial3SMC.get())
                    VectorPosicionFinalSMC_mm[2] = float(textoPosicionFinal3SMC.get())
                    VectorPasoSMC_mm[2] = float(textoPaso3SMC.get())
                if numeroDeSubintervalosSMC>=4:
                    VectorPosicionInicialSMC_mm[3] = float(textoPosicionInicial4SMC.get())
                    VectorPosicionFinalSMC_mm[3] = float(textoPosicionFinal4SMC.get())
                    VectorPasoSMC_mm[3] = float(textoPaso4SMC.get())
                if numeroDeSubintervalosSMC>=5:
                    VectorPosicionInicialSMC_mm[4] = float(textoPosicionInicial5SMC.get())
                    VectorPosicionFinalSMC_mm[4] = float(textoPosicionFinal5SMC.get())
                    VectorPasoSMC_mm[4] = float(textoPaso5SMC.get())
                if numeroDeSubintervalosSMC>=6:
                    VectorPosicionInicialSMC_mm[5] = float(textoPosicionInicial6SMC.get())
                    VectorPosicionFinalSMC_mm[5] = float(textoPosicionFinal6SMC.get())
                    VectorPasoSMC_mm[5] = float(textoPaso6SMC.get())
                if numeroDeSubintervalosSMC>=7:
                    VectorPosicionInicialSMC_mm[6] = float(textoPosicionInicial7SMC.get())
                    VectorPosicionFinalSMC_mm[6] = float(textoPosicionFinal7SMC.get())
                    VectorPasoSMC_mm[6] = float(textoPaso7SMC.get())
                if numeroDeSubintervalosSMC>=8:
                    VectorPosicionInicialSMC_mm[7] = float(textoPosicionInicial8SMC.get())
                    VectorPosicionFinalSMC_mm[7] = float(textoPosicionFinal8SMC.get())
                    VectorPasoSMC_mm[7] = float(textoPaso8SMC.get())
                if numeroDeSubintervalosSMC>=9:
                    VectorPosicionInicialSMC_mm[8] = float(textoPosicionInicial9SMC.get())
                    VectorPosicionFinalSMC_mm[8] = float(textoPosicionFinal9SMC.get())
                    VectorPasoSMC_mm[8] = float(textoPaso9SMC.get())
                if numeroDeSubintervalosSMC>=10:
                    VectorPosicionInicialSMC_mm[9] = float(textoPosicionInicial10SMC.get())
        ...
        botonSiguiente.grid(column=1, row=4)
        botonMenuPrincipal = tk.Button(raiz3, text = 'Menu', command = MenuPrincipal)
        botonMenuPrincipal.grid(column=0, row=4)
        raiz3.mainloop()
    
    def MediciónManual(self):
        raiz5 = tk.Tk()
        raiz5.title('Pump and Probe Software - Medición Manual')
        raiz5.geometry('1400x1000')
        ...
        def MenuPrincipal():
            raiz5.destroy()
            self.Continuar()
        botonMenuPrincipal = tk.Button(raiz5, text="Menu", command=MenuPrincipal)
    ...
    def SetearPuertos(self):
        ...
        botonMenuPrincipal = tk.Button(raiz4, text = 'Menu', command = MenuPrincipal)
    ...
    def Continuar(self):
        ...
        btn5 = tk.Button(raiz, text="Manual", command=MedicionManual)
